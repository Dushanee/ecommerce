...
<head>
    ...
    <link rel="stylesheet" href="../../../../public/customer/css/createEvent/venue_provider_shopPage.css">
    <script type="text/javascript" src="../../../../public/customer/js/outsidersView/outsider_venue_shoppage.js" defer></script>
...
<body>
    ...
    <div id="sp_info">
     ...
     <!-- <div id="spImg">
      ...
      require '../DBH.php';
        ...
        //$_SESSION['key'] = random_int(10000, PHP_INT_MAX);

        //echo $_SESSION['key']."<br>";

        //$Esp_id = encrypt_message($sp_id,$_SESSION['key']);
        //echo $Esp_id."<br>";
        
        //$sp_id = decrypt_message($Esp_id,$_SESSION['key']);
        //echo $sp_id."<br>";
      ...
      $sql = "SELECT * FROM packages INNER JOIN service_providers ON packages.sp_id = service_providers.sp_id WHERE service_providers.sp_type = 'venue' AND packages.pack_id = $pack_id";
    ...
    <div id="pack_info">
     ...
     $sql1 = "SELECT options.option_rate,options.option_description,venue_pack_options.capacity, option_type.type, options.optionPic1,options.optionPic2,options.optionPic3,options.optionPic4 FROM((options INNER JOIN venue_pack_options ON options.option_id = venue_pack_options.option_id) INNER JOIN option_type ON options.option_id = option_type.option_id) WHERE options.option_id = $min_option_id";
     //$sql1   = "SELECT * FROM options WHERE option_id = $min_option_id";
      ...
      if(mysqli_num_rows($result)>0){
          ...
          echo "</br>";
           if(isset($_GET['pack_id'])){
             ...
             $sql1 = "SELECT * FROM options INNER JOIN packages ON options.pack_id = packages.pack_id WHERE options.option_id = $option_id";
             $result1 = mysqli_query($conn, $sql1);
             ...
             if(mysqli_num_rows($result1)>0){
              ...
              $sql2 =  "SELECT options.option_rate,options.option_description,venue_pack_options.capacity, option_type.type, options.optionPic1,options.optionPic2,options.optionPic3,options.optionPic4 FROM((options INNER JOIN venue_pack_options ON options.option_id = venue_pack_options.option_id) INNER JOIN option_type ON options.option_id = option_type.option_id) WHERE options.option_id = $option_id";
